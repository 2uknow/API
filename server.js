...
function cleanupOldReports(){
  ...
  for (const f of fs.readdirSync(reportsDir)) {
    ...
    if (now - st.mtimeMs > maxAge) try { fs.unlinkSync(p); } catch {}
  }
}
...
app.get('/api/jobs', (req,res) => {
  ...
  try {
    if (!fs.existsSync(dir)) {
      console.error('[JOBS] dir missing:', dir);
      return res.json([]);
    }
    ...
    for (const f of files) {
      try {
        ...
        console.error('[JOBS] parse error:', f, e.message);
      }
    }
    res.json(items);
  } catch (e) {
...
app.get('/api/history', (req,res) => {
  try {
    ...
    res.json({ total: data.length, page, size, items, running: state.running });
...
app.post('/api/schedule', (req,res)=>{
  ...
  req.on('end', ()=>{
    try {
      ...
      saveSchedules();
      res.json({ ok:true });
    } catch (e) { res.status(400).json({ message:'invalid body' }); }
  });
});
app.delete('/api/schedule/:name', (req,res)=>{
  const { name } = req.params;
  const it = schedules.get(name);
  if (it){ it.task.stop(); schedules.delete(name); saveSchedules(); }
  res.json({ ok:true });
});
loadSchedules();

// robust spawn for Windows/Linux
function spawnNewmanCLI(args){
  let cmd, argv;
  if (process.platform === 'win32') {
    cmd = 'cmd.exe';
    argv = ['/d','/s','/c','npx', ...args]; // no quoting
  } else {
    cmd = '/bin/sh';
...
async function runJob(jobName){
  ...
  if (!fs.existsSync(collection)) {
    console.error('[API] collection not found:', collection);
    return { started:false, reason:'collection_not_found' };
  }
  if (environment && !fs.existsSync(environment)) {
    console.error('[API] environment not found:', environment);
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      ...
      let tags = [];
      try {
        const text = fs.readFileSync(cliExport, 'utf-8');
        const carriers = [...text.matchAll(/^\s*□\s*([A-Z_]+)\s*$/gm)].map(m => m[1]);
        tags = Array.from(new Set(carriers));
      } catch {}
      // history
      const history = histRead();
      history.push({
        timestamp: nowInTZString(),
        job: jobName,
        type: job.type,
        exitCode: code,
...
app.post('/api/run/:job', async (req,res) => {
  ...
  if (!result.started) {
    const map = { already_running:409, job_not_found:404, unsupported_type:400, collection_not_found:400, environment_not_found:400 };
